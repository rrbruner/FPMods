...
and computing with elements involves finding the enveloping profile.
    ...
    def DirectSum(alist,char=None):
        ...
        EXAMPLES::
            ...
            sage: from sage.modules.fpmods.fpmods import FP_Module
            sage: M = FP_Module([0],[[Sq(1)]])
            sage: N = M.suspension(4)
            sage: M_N = FP_Module.DirectSum([M,N]); M_N
        ...
        if len(alist) == 0 and char == None:
            ...
            alg = SteenrodAlgebra(p = alist[0].char,profile=\
                      Profile.enveloping_profile_profiles([x.profile() for x in alist],char))
            listdegs = reduce(lambda x,y: x+y, [x.degs for x in alist])
            numdegs = len(listdegs)
            listrels = [[0 for i in range(numdegs)] for k in range(\
...
class FP_Hom(Morphism):
    ...
    def __init__(self,domain,codomain,values,degree=0):
        ...
        self.algebra = SteenrodAlgebra(p = domain.algebra.prime(), profile = prof)
