...
#        fp_element_vp_time = 0.0
        ...
        for relation in self.j.values():
            ...
            for a in self.base_ring().basis(n - relation.degree()):
                xx = relation._lmul_(a)

#                dt = time.time()
                v = xx.vector_presentation()
#                fp_element_vp_time += time.time() - dt
...
#        quotient_time = time.time()
        Q_n = F_n/R_n
#        quotient_time = round(time.time() - quotient_time, 2)

#        total = time.time() - t
#        g_timings['fp_module.vp'] += total
...
#            timings['fp_module.vp'] += total
    ...
    def resolution(self, k, top_dim=None, verbose=False):
        ...
        for i in range(2, k+1):
            ...
            KK = FP_ModuleMorphism._resolve_kernel(
                f,
                top_dim=top_dim,
                verbose=verbose)
